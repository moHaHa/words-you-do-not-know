...
function App() {
  ...
  const handleOnLater = (plainText: string) => {
    ...
    const words = cleanedText
      .split(/\s+/)
      .filter((e) => e.trim() && !memorizedWords.includes(e));
    const unMemo = [...words, ...unMemorizedWords];
    setUnMemorizedWords(unMemo);
    localStorage.setItem('unMemorized', JSON.stringify(unMemo));
    setWordsLaterText('');
  };
    ...
    <>
      ...
      <Row gutter={[12, 12]}>
        <Col span={24}>
          <Card
            extra={
              <Button onClick={() => setShowUnMemorized(!showUnMemorized)}>
                Toggle
              </Button>
            }
            title={
              ...
              <div>
                <Space>
                  {unMemorizedWords.map((word) => (
                    <a
                      href={'https://youglish.com/pronounce/' + word}
                      target="_blank"
                    >
                      <Button>{word} </Button>
                    </a>
